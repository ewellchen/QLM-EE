...
class L2Norm_complex(Layer):

    def __init__(self, axis = 1, keep_dims = True, **kwargs):
        # self.output_dim = output_dim
        self.axis = axis
        self.keep_dims = keep_dims
        super(L2Norm_complex, self).__init__(**kwargs)

    def get_config(self):
        config = {'axis': self.axis, 'keep_dims': self.keep_dims}
        base_config = super(L2Norm_complex, self).get_config()
        return dict(list(base_config.items())+list(config.items()))
    ...
    def build(self, input_shape):
        ...
        #                               initializer='uniform',
        #                               trainable=True)
        super(L2Norm_complex, self).build(input_shape)  # Be sure to call this somewhere!
...
#        output = K.sqrt(K.sum(inputs**2, axis = self.axis, keepdims = self.keep_dims))
        output = K.sqrt(0.0000001+ K.sum(inputs[0]**2, axis = self.axis, keepdims = self.keep_dims) +
                        K.sum(inputs[1]**2, axis = self.axis, keepdims = self.keep_dims))
